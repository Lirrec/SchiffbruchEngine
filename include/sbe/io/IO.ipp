...
namespace sbe
{
	...
	std::vector<std::shared_ptr<T>> IO::loadFile(std::shared_ptr<IOPlugin>& IOP, const fs::path& p) {
		...
		if (IOP->loader_type == IOPlugin::loader::BINARY)
		{
			...
			auto BinIO = std::dynamic_pointer_cast<iBinaryIOPlugin<T>>(IOP);
		...
		{
			auto TreeIO = std::dynamic_pointer_cast<iTreeIOPlugin<T>>(IOP);
	...
	bool IO::saveObjects(Iter start, Iter end, bool ov) {
		...
		if (IOP->loader_type == IOPlugin::loader::PTREE)
		{
			auto tio = std::dynamic_pointer_cast<iTreeIOPlugin<T>>(IOP);
			pt::ptree tree;

			for (auto it = start; it != end; ++it)
			{
				if (tio->saveObject(ti.name(), **it, tree))
				{ // need some real name here instead of "some"... grrr.
